...
`;
  ...
  const eyeHidden = () => {
    $containerIcon[1].innerHTML = $eyeHidden;
    $input[1].setAttribute("type", "password");
  };
  eyeHidden();

  const openEye = () => {
    $containerIcon[1].innerHTML = $openEye;
    ...
    switch (e.target.name) {
      case "id":
        ...
        if (expressions.id.test(value)) {
          ...
          alert("El número debe tener máximo 14 caracteres.", 0);
          idStatus = false;
          globalStatus = false;
        }
        break;
      case "pwd":
        ...
        if (pwd.length < 6) {
          alertMessage("La contraseña debe tener mínimo 6 caracteres", 1);
          pwdStatus = false;
          globalStatus = false;
          ...
          alertMessage("La contraseña debe tener máximo 29 caracteres", 1);
          pwdStatus = false;
          globalStatus = false;
        }
        break;
  ...
  let eye = false;
  $containerIcon[1].addEventListener("click", () => {
    if (eye) {
      eyeHidden();
      eye = false;
    } else {
